...
class SageConnection(BackendConnection):
    def __init__(
        self, name, conffile="sage.conf", prefix_conffile=False, *args, **kwargs
    ):
        ...
        if prefix_conffile:
            ...
            host_rank_conffile = os.path.join(
                conffile_dir, "{}.{}.{}".format(host, rank, conffile_name)
            )
            rank_conffile = os.path.join(
                conffile_dir, "{}.{}".format(rank, conffile_name)
            )
        ...
        if not os.access(conffile, os.R_OK):
            raise ConnectionError("Configuration file {} not found".format(conffile))
...
class SageDataset(BackendDataset):
    ...
    def create_chunk(self, idx, data=None, slices=None):
        ...
        log.debug("creating datachunk %s with idx:%s shape:%s", name, idx, shape)
    ...
    def has_chunk(self, idx):
        return self.connection.clovis.has_object_chunk(self.name, self._idx2name(idx))

    def del_chunk(self, idx):
        log.debug("deleting datachunk from %s with idx:%s", self.name, idx)
        try:
            self.connection.clovis.delete_object_chunk(self.name, self._idx2name(idx))
        except KeyError:
            logging.debug(
                "chunk %s with idx:%s was not found while deleting", self.name, idx
            )
            return False
...
class SageDataChunk(BackendDataChunk):
    ...
    def write(self, data):
        self.dataset.connection.clovis.write_object_chunk(
            self.dataset.name, int(self.name), data, len(data)
        )
