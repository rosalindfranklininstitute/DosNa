...
class H5Connection(BackendConnection, DirectoryTreeMixin):
    ...
    def create_dataset(
        ...
        if self.has_dataset(name):
            raise Exception("Dataset at `{}` already exists".format(self.relpath(name)))
        ...
        os.mkdir(path)
        with h5.File(self._get_dataset_metadata_path(name), "w") as file_handle:
        ...
        dataset = H5Dataset(self, name, shape, dtype, fillvalue, chunk_grid, chunk_size)
    ...
    def get_dataset(self, name):
        ...
        with h5.File(self._get_dataset_metadata_path(name), "r") as file_handle:
        ...
        return H5Dataset(self, name, shape, dtype, fillvalue, chunk_grid, chunk_size)

    def has_dataset(self, name):
        return os.path.isdir(self.relpath(name)) and os.path.isfile(
            self._get_dataset_metadata_path(name)
        )

    def del_dataset(self, name):
        path = self.relpath(name)
        if not self.has_dataset(name):
            raise DatasetNotFoundError("Dataset at `{}` does not exist".format(path))
...
class H5Dataset(BackendDataset, DirectoryTreeMixin):
    ...
    def create_chunk(self, idx, data=None, slices=None):
        ...
        return H5DataChunk(
            self, idx, chunk_name, self.chunk_size, self.dtype, self.fillvalue
        )
